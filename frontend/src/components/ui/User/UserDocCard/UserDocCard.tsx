...
const UserDocCard = ({ props }: { props: ITableResponse }) => {
  return (
    <div className="w-full bg-base-100 p-4 rounded-[9px] flex justify-between items-center">
      ...
      <Link className='flex gap-2 items-center bg-primary text-base-100 font-medium rounded-md px-2 py-1' to={`https://novis.ddns.net${props.file}`} target="_blank" rel="noopener noreferrer" download>
        Скачать
        <svg
          width="24"
          height="25"
